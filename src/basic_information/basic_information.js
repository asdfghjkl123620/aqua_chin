...
router.post('/multiple_test',upload.fields([{ name: 'seller_img', maxCount: 1 }, { name: 'shop_img', maxCount: 8 }]),(req,res)=>{
// console.log(req.files['seller_img'][0].originalname);
// console.log(req.files['shop_img']);
req.files['shop_img'].forEach(function (arrayItem,index) {
//    arrayItem.originalname
    console.log( 'S' + moment(new Date()).format('YYYYMMDDHHmmss') + '.' +index+ arrayItem.originalname.split('.')[1]);
});
})
